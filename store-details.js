(()=>{
  ...
  const openMonth=(name,key)=>{
    const list=storeRows(name).filter(r=>mk(rowDate(r))===key).sort((a,b)=>(rowDate(a)?.getTime()||0)-(rowDate(b)?.getTime()||0));
  ...
  const openStore=name=>{
    ...
    document.getElementById('storeMonthTable').innerHTML=me.length?'<table><tr><th>Miesiąc</th><th>Paragony</th></tr>'+me.map(([k,n])=>`<tr class="store-month-row" data-month="${esc(k)}" style="cursor:pointer"><td><button style="padding:0;border:0;background:transparent;color:var(--a);font-weight:700;cursor:pointer">${ml(k)}</button></td><td><b>${n}</b></td></tr>`).join('')+'</table>':'<div class="empty">Brak danych miesięcznych.</div>';
