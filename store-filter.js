// deployment marker: indexed store year filters + cached yearly aggregates + rAF search
(()=>{
  const PAGE=30;
  let query='',year='',shown=PAGE,lastList=[],lastTotal=0,navRaf=0,searchRaf=0,lastRowsSig='';
  let cacheData=null,yearEntriesCache=new Map(),lowerNameCache=new Map(),totalCache=new Map();
  const index=()=>window.PanParagonMainIndex?.get?.()||null;
  const resetCache=data=>{cacheData=data;yearEntriesCache=new Map();lowerNameCache=new Map();totalCache=new Map()};
  const ensureCache=data=>{if(data!==cacheData)resetCache(data)};
  const lower=name=>{const key=String(name||'');if(lowerNameCache.has(key))return lowerNameCache.get(key);const v=key.toLocaleLowerCase('pl');lowerNameCache.set(key,v);return v};
  const availableYears=()=>{const data=index();if(data?.yearEntries)return data.yearEntries.map(([y])=>String(y));return []};
  const ensureControls=()=>{
    ...
    table.before(box);
    document.getElementById('storeSearch').addEventListener('input',e=>{query=e.target.value.trim().toLocaleLowerCase('pl');shown=PAGE;lastRowsSig='';cancelAnimationFrame(searchRaf);searchRaf=requestAnimationFrame(renderFiltered)});
  ...
  const refreshYears=()=>{
    ...
    if(s.dataset.yearsSig!==sig){s.innerHTML='<option value="">Wszystkie lata</option>'+list.map(y=>`<option value="${y}">${y}</option>`).join('');s.dataset.yearsSig=sig}
  ...
  const storeEntries=()=>{
    ...
    ensureCache(data);
    const key=year||'*';
    if(yearEntriesCache.has(key))return yearEntriesCache.get(key);
    let entries;
    if(!year)entries=Array.isArray(data.ae)?data.ae:Object.entries(data.allStores||{}).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'pl'));
    else{
      const out={};
      ...
      entries=Object.entries(out).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'pl'));
    }
    yearEntriesCache.set(key,entries);
  ...
  const wireMoreButton=()=>{
    ...
    if(!wrap){wrap=document.createElement('div');wrap.id='storeLoadMoreWrap';wrap.style.cssText='text-align:center;padding:14px 0 2px';const btn=document.createElement('button');btn.id='storeLoadMore';wrap.appendChild(btn);box.appendChild(wrap)}
    const btn=wrap.querySelector('#storeLoadMore');if(btn){btn.textContent=`Pokaż więcej (${Math.min(PAGE,remaining)})`;btn.onclick=()=>{shown+=PAGE;renderRows();updateInfo()}}
  };
  const renderRows=()=>{
    const box=document.getElementById('storesTable');if(!box)return;
  ...
  const updateInfo=()=>{const info=document.getElementById('storeFilterInfo');if(!info)return;const visible=Math.min(shown,lastList.length),text=`Znaleziono sklepów: ${lastList.length} · pokazano: ${visible} · paragony: ${lastTotal}${year?' · rok '+year:' · wszystkie lata'}${query?' · wyszukiwanie: „'+query+'”':''}.`;if(info.textContent!==text)info.textContent=text};
  const renderFiltered=()=>{
    ensureControls();refreshYears();const base=storeEntries(),key=year||'*';
    lastList=query?base.filter(([name])=>lower(name).includes(query)):base;
  ...
  const install=()=>{
    ensureControls();if(document.getElementById('stores')?.classList.contains('on')){if(!primeExistingTable())renderFiltered()}
    document.addEventListener('click',e=>{if(e.target.closest?.('#nav button[data-v="stores"]'))scheduleNavRender()});
    document.addEventListener('panparagon:data-changed',e=>{if(e?.detail?.reason==='main-render-fast'||e?.detail?.source==='main-render-fast')return;resetCache(index());lastRowsSig='';refreshYears();if(document.getElementById('stores')?.classList.contains('on')){shown=PAGE;renderFiltered()}});
  };
