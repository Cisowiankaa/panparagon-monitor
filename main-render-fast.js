(()=>{
  ...
  const renderFilter=selected=>{
    ...
    $('rc').textContent=count;$('sc').textContent=rank.length;$('mc').textContent=base.me.length;$('top').textContent=rank[0]?.[0]||'—';$('topm').textContent=rank[0]?`${rank[0][1]} paragonów`:'Brak danych';
  ...
  const renderStatic=selected=>{
    const {months,years,undated,me,yearEntries,ae}=base;
